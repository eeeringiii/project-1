 ...
 *
 * ※ 本モジュールは他の配信サービス（publishing.ts）同様サービス層であり、
 *   認証情報（NOTE_EMAIL/NOTE_PASSWORD）を読むため **サーバー/ワーカー限定** で import する。
 *   クライアントからは import しないこと（非 NEXT_PUBLIC_ の env はクライアントに載らない）。
 *
