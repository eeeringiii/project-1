...
const LABEL = '今日の推し活運勢';
const HASHTAG = '#今日の推し活運勢';
...
export async function GET(request: Request) {
  ...
  const meta = `${fortune.displayDate}${type ? `　/　${type.code} ${type.name}` : ''}`;

  return new ImageResponse(
    <div style={{ height: '100%', width: '100%', display: 'flex', padding: 44, background: fortune.level.tone, fontFamily: OG_FONT_STACK }}>
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', flex: 1, background: '#ffffff', border: '5px solid #3a2a4d', borderRadius: 44, padding: 60 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ marginLeft: 20, fontSize: 26, color: '#7c6b90' }}>{LABEL}</span>
        </div>
        <span style={{ fontSize: 28, color: '#7c6b90', marginTop: 26 }}>{meta}</span>
        ...
        <span style={{ fontSize: 26, color: '#ff6fa3', marginTop: 30, fontWeight: 700 }}>{HASHTAG}</span>
      </div>
    </div>,
    await ogImageOptions(BRAND, LABEL, meta, fortune.level.name, fortune.headline, HASHTAG),
  );
}
