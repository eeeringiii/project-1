...
// 診断結果をシェアしたときに実際に開かれるのがこのページ。
// タイプごとのOGカードを出さないと、どのタイプを共有しても同じ画像になってしまう。
export async function generateMetadata({ params }: { params: Promise<{ typeCode: string }> }): Promise<Metadata> {
  ...
  if (!type) return {};
  const title = `${type.code}｜${type.name}`;
  const description = `${type.catchphrase} ${type.name}の生息地・主食・特殊能力・弱点まで。あなたのヲタク生態も24問で解析できます。`;
  const image = ogImageUrl(type.code);
  return {
    title,
    description,
    alternates: { canonical: `/types/${type.code}` },
    openGraph: { title: `${title}｜OSHICOA 16`, description, url: `/types/${type.code}`, images: [{ url: image, width: 1200, height: 630 }] },
    ...
    <main className="shell">
      ...
      <div className="resultGrid">
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <OfferCta type={type} placement="type" />
        </div>
      </div>
    </main>
  );
